...
sc.lat = 180*np.random.random(Nthrow)
...
CR_r0, CR_dir, CR_lat, CR_long = throwCR(sc, R + sc.altitude, Nthrow)
#print(CR_r0, CR_dir)
b2 = CR_r0[:, 0]**2 + CR_r0[:, 1]**2 + CR_r0[:, 2]**2 - (CR_r0[:, 0]*CR_dir[:, 0] + CR_r0[:, 1]*CR_dir[:, 1] + CR_r0[:, 2]*CR_dir[:, 2])**2
b2max = np.max(b2)
b2min = np.min(b2)
mask = (b2 < (R + 100)**2) & (b2 > (R - 100)**2) 
event = projectCRCone(sc, CR_r0[mask], CR_dir[mask], R + sc.altitude, mask)
