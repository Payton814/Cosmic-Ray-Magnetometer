...
def projectCRCone(sc, CR_r0, CR_dir, R = 25362):
    ...
    sc_x = (R + sc.altitude)*np.sin(sc.lat*np.pi/180)*np.cos(sc.long*np.pi/180)
    sc_y = (R + sc.altitude)*np.sin(sc.lat*np.pi/180)*np.sin(sc.long*np.pi/180)
    sc_z = (R + sc.altitude)*np.cos(sc.lat*np.pi/180)
